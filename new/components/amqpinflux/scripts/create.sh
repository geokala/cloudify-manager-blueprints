...
inject_service_env_var "{{ ctx.node.properties.rabbitmq_username }}" "${RABBITMQ_USERNAME}" "amqpinflux"
inject_service_env_var "{{ ctx.node.properties.rabbitmq_password }}" "${RABBITMQ_PASSWORD}" "amqpinflux"
